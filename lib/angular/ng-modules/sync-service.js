...
.run(function(mediator, resultSync) {
  mediator.subscribe('appform:submission:complete', function(event) {
    resultSync.managerPromise
    .then(function(manager) {
      return manager.getByWorkorderId(event.metaData.workorderId)
    })
    .then(function(result) {
      var stepResult = result.stepResults[event.metaData.step.code];
      stepResult.submission = event.submissionResult;
